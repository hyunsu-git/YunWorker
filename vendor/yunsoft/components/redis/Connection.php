...
namespace yun\components\redis;

use yun\exception\DatabaseException;
use yun\exception\InvalidArgumentException;
use yun\exception\RedisException;
use yun\helpers\ArrayHelper;
use yun\helpers\LangHelper;

/**
 * Class Connection
 * 用户连接到redis的类,支持在配置中设置密码和选择数据库
 * 默认使用6379端口,使用编号为0的数据库
...
class Connection
{
    ...
    public function open()
    {
        ...
        if ($this->_socket) {
            ...
            throw new DatabaseException($message, $errorDescription, $errorNumber);
        }
    }
    ...
    {
        $redisCommand = strtoupper(ArrayHelper::camel2words($name, false));
        if (in_array($redisCommand, $this->redisCommands)) {
            return $this->executeCommand($redisCommand, $params);
        } else {
            throw new InvalidArgumentException(LangHelper::ts("Non-existent or unsupported command:%s", $redisCommand));
        }
    }
    ...
    private function sendCommandInternal($command, $params)
    {
        ...
        if ($written === false) {
            throw new RedisException(LangHelper::ts('Failed to write to socket,Redis command was:%s', $command));
        }
    ...
    private function parseResponse($command)
    {
        if (($line = fgets($this->_socket)) === false) {
            throw new RedisException('Failed to read to socket,Redis command was:%s', $command);
        }
        $type = $line[0];
        ...
        switch ($type) {
            ...
            case '*': // Multi-bulk replies
                ...
                return $data;
            default:
                throw new RedisException(LangHelper::ts('Received illegal data from redis:%s,Redis command was:%s', $line, $command));
        }
    }
}